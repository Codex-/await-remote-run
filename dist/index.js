...
async function getWorkflowRunFailedJobs(runId) {
  ...
  Jobs: [${jobs.map((job) => job.name)}]`);
    ...
    if (error3 instanceof Error) {
      core2.error(`getWorkflowRunJobFailures: An unexpected error has occurred: ${error3.message}`);
      error3.stack && core2.debug(error3.stack);
    }
    throw error3;
  }
...
async function getWorkflowRunActiveJobUrl(runId) {
  ...
  Jobs (in_progress): [${fetchedInProgressJobs.map((job) => job.name)}]`);
    ...
    if (error3 instanceof Error) {
      core2.error(`getWorkflowRunActiveJobUrl: An unexpected error has occurred: ${error3.message}`);
      error3.stack && core2.debug(error3.stack);
    }
    throw error3;
  }
...
async function logFailureDetails(runId) {
  ...
  for (const failedJob of failedJobs) {
    ...
    core3.error(`Job ${failedJob.name}:
  ID: ${failedJob.id}
...
async function run() {
  try {
    ...
    core3.info(`Awaiting completion of Workflow Run ${config2.runId}...
  ID: ${config2.runId}
  ...
  Conclusion: ${conclusion}`);
      ...
      await new Promise((resolve) => setTimeout(resolve, config2.pollIntervalMs));
    }
